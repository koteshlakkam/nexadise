...
console.log(
	"OpenRouter key:",
	Boolean(orKey),
	orKey ? `(${orKey.length} chars)` : "",
);
console.log("OpenRouter model:", orModel);
console.log(
	"Gemini key:",
	Boolean(geminiKey),
	geminiKey ? `(${geminiKey.length} chars)` : "",
);
...
if (orKey) {
	...
	try {
			...
			"https://openrouter.ai/api/v1/chat/completions",
			{
				...
				body: JSON.stringify({
					...
					messages: [
						{
							role: "user",
							content: 'Reply with only the JSON {"ok": true}.',
						},
					],
		...
		if (!res.ok) {
			...
			console.log(
				"https://openrouter.ai/models?max_price=0 and set OPENROUTER_MODEL in .env.",
			);
			process.exit(1);
		}
	...
	try {
		...
		const res = await model.generateContent({
			contents: [
				{
					role: "user",
					parts: [{ text: 'Reply with only the JSON {"ok": true}.' }],
				},
			],
...
console.log(
	"Add OPENROUTER_API_KEY=sk-or-... to .env (free at https://openrouter.ai/keys).",
);
process.exit(1);
