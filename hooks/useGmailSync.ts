...
async function readError(res: Response): Promise<ParsedError> {
  ...
  try {
    ...
    if (parsed && typeof parsed === "object") {
      const message = "error" in parsed ? String((parsed as { error: unknown }).error) : text;
      const code =
...
export function useGmailSync(onAfterSync?: (summary: SyncSummary) => void | Promise<void>) {
    ...
    async (opts: SyncOptions = {}) => {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          if (parsed.code === "RefreshAccessTokenError" || parsed.code === "GmailUnauthorized") {
            setNeedsReauth(true);
          }
  ...
  return { sync, isSyncing, error, needsReauth, reauth, summary, dismissSummary };
}
