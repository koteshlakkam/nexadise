...
export type EmailActionType = "REPLY" | "REVIEW" | "APPROVAL" | "MEETING" | null;
...
function isProcessedEmailLike(x: unknown): x is Omit<ProcessedEmail, "source"> {
  ...
  const summaryOk = typeof o.summary === "string" && o.summary.trim().length > 0;
  const suggestedOk =
    typeof o.suggestedAction === "string" && o.suggestedAction.trim().length > 0;
  ...
  const urgencyOk = o.urgency === "LOW" || o.urgency === "MEDIUM" || o.urgency === "HIGH";
...
export async function probeAi(): Promise<
  ...
  | { ok: false; provider: ProviderName | "none"; model: string; error: string; hint?: string }
> {
  refreshConfigSnapshot();
  ...
  if (provider === "none") {
    return {
      ...
      hint:
        "Add OPENROUTER_API_KEY=sk-or-... to .env (free at https://openrouter.ai/keys), then restart the dev server.",
    };
  }
  ...
  try {
    const raw =
      provider === "openrouter" ? await callOpenRouter(prompt) : await callGemini(prompt);
    return {
      ok: true,
      provider,
    ...
    if (/no endpoints|not found|404|unsupported/i.test(msg)) {
      ...
      hint = "Provider is overloaded. Try again, or switch model via env.";
    } else if (/quota|429|rate/i.test(msg)) {
      hint = "Rate limit / quota hit. Wait a moment, or switch to a different free model.";
    } else if (/fetch failed|enotfound|econnrefused|network/i.test(msg)) {
      hint = "Network can't reach the provider. Check internet/proxy/firewall.";
    }
    return { ok: false, provider, model: stats[provider].model, error: msg, hint };
  }
}
...
function ruleBasedFallback(subject: string, snippet: string): ProcessedEmail {
  ...
  const actionRequired = ["meeting", "call", "reply", "urgent", "approve", "review"].some(
    (k) => text.includes(k),
  );
  ...
  if (["meeting", "call", "schedule"].some((k) => text.includes(k))) actionType = "MEETING";
    ...
    actionType = "APPROVAL";
  else if (["check", "verify", "feedback", "review"].some((k) => text.includes(k)))
    actionType = "REVIEW";
  else if (["respond", "reply"].some((k) => text.includes(k))) actionType = "REPLY";

  const urgency: EmailUrgency = ["urgent", "today", "asap", "deadline"].some((k) =>
    text.includes(k),
  )
    ? "HIGH"
    : actionRequired
